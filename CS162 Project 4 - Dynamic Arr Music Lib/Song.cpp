...
Song::Song(const char title[], const char artist[], const char duration[], const char album[])
{
	...
	/*
	*Notes:
	A member function of a class can (directly) access the member variables of a fiven object of that class.
...
out: title
*/
void Song::getTitle(char title[]) const //setting this class member function as const makes it unable to change the values of any of the other class member variables. Also const mem functions can only call other const mem functions of the same class.
{
...
*/
void Song::print() const //const, so it can't change the values it calls, and (different accounts from stack overflow...) a const member function can only return a cont pointer or reference to a member <- true?
{
